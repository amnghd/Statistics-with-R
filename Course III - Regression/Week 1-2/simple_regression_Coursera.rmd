...
ggplot(data= mlb11, aes(x= at_bats, y= runs))+
  ...
  ggtitle('Scatter plot of runs vs at bats')+
  xlab('at bats')+
  ylab('runs')
```
...
There is much scatter around the possible linear line. Therefore, it seems we need to perform an inference test to assure the possible linear relationship actually exist. 
</div>
...
<li> The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats. Answer</li>
...
<ol>
<li> For each additional home run, the model predicts 1.83 more runs, on average- Answer. </li>
...
Having the formula $\hat{y} = -2789.2429 + 0.6305 \times at\_bats = 728.35$ we can see that
...
value of the `runs variable for that observation. The difference between the observed and predicted is $y-\hat{y} = 713-728= -15$
...
<li> The residuals show a curved pattern-Answer </li> 
...
ggplot(data= mlb11, aes(x= new_onbase, y= runs))+
  ...
  ggtitle('Scatter plot of runs vs wins')+
  xlab('wins')+
  ylab('runs')
...
At bats' R2 square is 0.63 while new on base's is 0.85  which demonstrate that higher variability in the data is explained by this parameter.
</div>
```{r pick-model}
m3 <- lm(runs ~ wins, data = mlb11)
summary(m3)
...
**Linearity-Check**: You already checked if the relationship between runs and at-bats
is linear using a scatterplot. We should also verify this condition with a plot 
of the residuals vs. fitted (predicted) values.
...
ggplot(data = m4, aes(x = .resid)) +
  geom_histogram(binwidth = 25, fill = 'darkorchid4') +
  xlab("Residuals")
```
