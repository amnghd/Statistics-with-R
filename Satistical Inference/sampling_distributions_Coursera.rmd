...
the e and store each result in a vector called 
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50%>%summarise(mean=mean(x_bar), Std_error=sd(x_bar))
```
...
# type your code for the Exercise here, and Run Document
i=1
sample_means_small<-data.frame(matrix(ncol = 2, nrow = 25))
names(sample_means_small)<-c("replicate", "x_bar")
while (i<26) {
  sample_means_small[i,1]<-i
  sample_means_small[i,2]<-mean((ames%>%sample_n(size=10))$area)
  i=i+1
}
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means_50_2<-ames%>%rep_sample_n(size=50, reps=5000, replace=TRUE)%>%summarise(price_bar=mean(price))

mean(sample_means_50_2$price_bar)

ggplot(data=sample_means_50_2, aes(x=price_bar))+geom_histogram(bins=20)
```
...
# type your code for this Exercise here, and Run Document
sample_means_150_2<-ames%>%rep_sample_n(size=150, reps=5000, replace=TRUE)%>%summarise(price_bar=mean(price))

mean(sample_means_150_2$price_bar)

ggplot(data=sample_means_150_2, aes(x=price_bar))+geom_histogram(bins=20)
```
...
# type your code for this Exercise here, and Run Document
ames%>%sample_n(size=15)%>%summarise(mean_price=mean(price))
...
# type your code for this Exercise here, and Run Document
sample_means_15<-ames%>%rep_sample_n(size=15, reps=2000, replace=TRUE)%>%summarise(price_bar=mean(price))

mean(sample_means_15$price_bar)

ggplot(data=sample_means_15, aes(x=price_bar))+geom_histogram(bins=20)
```
...
# type your code for this Exercise here, and Run Document
sample_means_150_3<-ames%>%rep_sample_n(size=150, reps=2000, replace=TRUE)%>%summarise(price_bar=mean(price))

mean(sample_means_150_3$price_bar)

ggplot(data=sample_means_150_3, aes(x=price_bar))+geom_histogram(bins=20)
```
