...
```{r}
print('fage, mage, weeks, visits, gained, weight are numerical')

print('mature, premise, marital, lowbirthweight, gender, habit, and whitemom are ')
```
Let's see the  outilier visualization for gained, and mage.

```{r}
ggplot(nc%>%na.omit(), aes(x=marital, y=mage))+
  geom_boxplot(color='blue')+
  facet_grid(habit~whitemom)
```
...
ggplot(data=nc%>%na.omit(), aes(x=habit, y=weight))+geom_boxplot(color='red')

```
```{r}
ggplot(data=nc%>%na.omit(), aes(x=weight))+geom_density(kernel='gaussian')
```
...
1- Independance : since the data is gathered from a reputed agency, we can assume that the random sampling is performed. Moreover, the data size (716, 84) is smaller than 10% of births.
2- Sample size/skew : population is skewed, however, both data samplesa are larger than 30.
</div>
...
print ("The width of the CI is smaller in this case compared to 99% case as this one is less concervative.")
```
...
print("Obviously the mean weight of young mother babies are not statistically different from mature mothers.")
```
...
nc%>%na.omit()%>%filter(mature == 'mature mom')%>%summarise(min_mature = min(fage))

nc%>%na.omit()%>%filter(mature == 'younger mom')%>%summarise(max_young = max(fage))


nc%>%na.omit()%>%summarise(father_age_mean = mean(fage), father_age_min = min(fage), father_age_max = max(fage))
...
** Question**:
Is there any difference between the father age of boy babies and girls babies?
First let's see it EDA and VEDA wise. There seems to be a small difference in medians. Let's check if the median's difference is meaningful.

Checking conditions:
1- Independanace: Being a reputed instuition we can assume data is selected randomly. Moreover, number almost 400 is less than boy's fathers and girl's fathers population.
```{r}
ggplot(data=nc%>%na.omit(), aes(x=gender, y=fage))+geom_boxplot(color='green')
nc %>%
  na.omit()%>%
  group_by(gender) %>%
  summarise(median_father_age = mean(fage), size=n())
```
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(y = fage, x = gender, data = nc, statistic = "median", type = "ht", null = 0, 
          alternative = "twosided", method = "simulation", boot_method = "perc")
```
