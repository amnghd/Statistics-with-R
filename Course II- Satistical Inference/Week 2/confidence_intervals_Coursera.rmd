...
mean_samp = mean(samp$area)
SE = sd(samp$area)/sqrt(length(samp$area))
print(mean)
print(SE)

ggplot(samp, aes(area, fill = area))+geom_density(kernel = 'gaussian', color = 'red', alpha = 0.2, fill = "#ff4d4d")+geom_histogram(aes(y = ..density..), alpha = 0.7, fill = "#333333",bins=20) 
```
...
# type your code for the Exercise here, and Knit
params>summary$lower & params<summary$upper
```
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
cci_data <- data.frame(cci_id = c(1:50, 1:50), cci_bounds = c(cci$lower, cci$upper),
                       capture_mu = c(cci$capture_mu, cci$capture_mu))




ggplot(data=cci_data, aes(x = cci_bounds, y= cci_id,
                          group = cci_id, color = capture_mu)) +
  geom_point(size = 2) +
  geom_line() +
  geom_vline(xintercept = params$mu, color = "black")
